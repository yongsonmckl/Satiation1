...
- Phase 4: Partially done
  - Gemini key storage/settings are done
  - Camera preview exists
  - Real capture, image handoff, robust parsing, and save flow are still not done
...
- Phase 7: Partially done
  - Build checks and Phase 3 smoke/usability testing were completed
  - Camera/Gemini verification is still incomplete
...
Still required:
- [ ] Add actual CameraX image capture, not only preview
- [ ] Convert the captured image into a Gemini-supported image input
- [ ] Populate `SatiationViewModel.capturedImage`
- [ ] Make camera -> nutrition flow depend on a real captured image
- [ ] Harden Gemini response parsing by stripping fences and validating fields
- [ ] Expand Gemini output to include totals plus item-level macros/confidence
- [ ] Add review/edit-before-save behavior
- [ ] Save accepted AI results to Room as `MealLog` + `MealItem`
- [ ] Add user-facing error states for missing key, no image, API failure, and malformed output
...
Still required:
- [ ] Smoke test camera permission and real image capture
- [ ] Smoke test Gemini scan with a valid user-provided API key
- [ ] Verify AI save flow updates Home, Daily Targets, and Progress
...
[ ] 8. Better AI review screen before saving
